 ...
 *  The best way to understand it's behavior is via example: A buffer made of 3 parts of 5 floats.
 * This means the whole buffer is made of 15floats:
 ...
 *
 * @Note: User never needs to care about where the partition is actually pointing, this is automatically handled. 
...
public:
  ...
  /**
   *  Fills a partition with the given data. This is a very fast memory movement. 
  ...
  const _Type* GetPartitionDataConst(std::uint8_t index)const { return GetPartitionPointer(index);  }

  /**
   * Get pointer to the partition
   * @param index to retrieve data (without shifting)
   */
  _Type* GetPartitionData(std::uint8_t index)const { return GetPartitionPointer(index);  }
   ...
   */
  [[nodiscard]]std::array<_Type, _Size> GetPartitionDataAsArray(std::uint8_t index){
...
[[nodiscard]] _Type* GetPartitionPointer(std::uint8_t index) const {
        return (isIndexInRange(index))
            ? mBuffer.begin() + (ShiftIndex(index) * _Size)
            : nullptr;
    }

  constexpr bool isIndexInRange(std::uint8_t index) {
    return index < _Partitions;
  }
